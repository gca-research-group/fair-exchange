...
def handshake(token: str):
    ...
    response = requests.post(
        f"http://localhost:5001/api/exchange/", json={"token": token}
    )

    if response.status_code != 201:
        return {"error": "Failed to perform handshake with Bob"}, response.status_code
...
def send(token: str):
    ...
    response = requests.post(
        f"http://localhost:5001/api/exchange/{token}/item/", data=encrypted_item
    )
...
def item(token: str):
    base_dir = f"attestable/{token}/bob"
    os.makedirs(base_dir, exist_ok=True)
