...
ATTESTABLE_URL = "http://localhost:7001/api/exchange"
APP_A_URL = "http://localhost:6000/api/exchange"
...
def send_public_key(exchange_token: str):
    with scoped_connection() as connection:
        ...
        response = requests.patch(
            f"{APP_A_URL}/{exchange_token}/public-key",
            json=payload,
        )
...
def send(exchange_token: str):
    ...
    with scoped_connection() as connection:
        ...
        response = requests.post(
            f"{APP_A_URL}/{exchange_token}/send-attestable",
            json=payload,
        )
...
def send_to_attestable(exchange_token: str):
    ...
    with scoped_connection() as connection:
        ...
        if not exchange.public_key:
            return "Public key not set for the exchange", 400
...
def handshake(exchange_token: str):
    ...
    with scoped_connection() as connection:
        ...
        if exchange:
            return {"error": f"There is already an exchange with this token: {exchange_token}"}, 400
